...
import { requestEnvironmentalAlertsRefresh } from '../constants/health-alerts-refresh'
import {
...
import {
  ...
  ENVIRONMENTAL_SAFETY_BANNER_LINES,
...
export type EnvironmentalAlertDetailData = {
  id: string
  lat: number
  lng: number
  createdAt: string
...
type EnvironmentalAlertCommentItem = {
  commentId: string
  content: string
  createdAt: string
  authorId: string
...
type EnvironmentalAlertDetailDialogProps = {
  open: boolean
  onOpenChange: (open: boolean) => void
  alert: EnvironmentalAlertDetailData | null
  refLat?: number
  refLng?: number
}

export function EnvironmentalAlertDetailDialog({
  open,
  onOpenChange,
  alert,
  refLat,
  refLng,
}: EnvironmentalAlertDetailDialogProps) {
  const { user } = useUser()
  ...
  const [comments, setComments] = useState<EnvironmentalAlertCommentItem[]>([])
    ...
    async (options?: { silent?: boolean }) => {
      ...
      try {
        ...
        const payload = (await response.json()) as {
          comments?: EnvironmentalAlertCommentItem[]
              ...
              <DialogHeader className="shrink-0 gap-3 p-0">
                ...
                >
                  <div className="grid h-[37px] w-[37px] shrink-0 place-items-center rounded-[73px] border-2 border-white bg-[#F4F4F4]">
                    <img
                      src={`${ENVIRONMENTAL_ICON_BASE}/${alert.iconSlug}.svg`}
                      alt=""
                      title={alert.title}
                ...
                <DialogDescription className="sr-only">
                  Detalhes do alerta ambiental registrado no mapa.
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto [scrollbar-width:thin] [scrollbar-color:rgba(19,81,180,0.25)_transparent] [&::-webkit-scrollbar]:w-[5px] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[rgba(19,81,180,0.25)]">
                <div className="px-4 pb-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:px-5">
                    ...
                    {!emptyImageComposerMode ? (
                      ...
                      >
                        <button
                          ...
                          aria-controls="environmental-alert-comments-panel"
                  ...
                  {!emptyImageComposerMode && isCommentsOpen ? (
                    <div
                      id="environmental-alert-comments-panel"
                      className="mt-3"
                      role="region"
